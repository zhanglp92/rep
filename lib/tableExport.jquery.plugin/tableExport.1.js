  ...
  $.fn.tableExport = function (options) {
    ...
    if (defaults.type === 'csv' || defaults.type === 'tsv' || defaults.type === 'txt') {
      ...
      $(el).filter(function () {
        ...
        ranges = [];
        colNames = GetColumnNames(this);
        colNumber = colNames.length - defaults.ignoreColumn.length;

        // Header
        docData += '<table><thead>';
        docData += defaults.extExcelHeader(colNumber);
          ...
          ForEachVisibleCell(this, 'th,td', rowIndex, $hrows.length,
            function (cell, row, col) {
              if (cell !== null) {
                ...
                trData += '>' + tdValue + '</th>';
                defaults.extExcelDo($extData, row, col, tdValue);
              }
            });
          if (trData.length > 0)
        ...
        docData += defaults.extExcelTail($extData, colNumber);
        docData += '</tbody></table>';
      });

      // 自定义docdata
      docData = defaults.extExcelDocData($extData);
